...
func Run(config *Config, join, version string) (*messaging.Broker, *influxdb.Server, *raft.Log) {
	...
	if err != nil {
		log.Fatalf("server failed to listen on %s. %s ", config.BrokerAddr(), err)
	}
	go func() {
		err := http.Serve(listener, h)
		if err != nil {
			log.Fatalf("server failed to server on %s.: %s", config.BrokerAddr(), err)
		}
	}()
	log.Printf("server listening on %s", config.BrokerAddr())
